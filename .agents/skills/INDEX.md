...
## 🖥️ System Terminal Policy

> **Все терминальные команды ОБЯЗАНЫ иметь префикс `rtk `** (напр. `rtk git status`, `rtk ls -la`, `rtk go test ./...`).
>
> Это включает автоматическое сжатие вывода для проекта. Любая команда без `rtk ` — нарушение.
...
| **Упражнения по дженерикам Go** | `generics/` (напр. `generics/Filter/main_test.go`) — сырой текст задачи → формулировка в стиле LeetCode |
| Архив: Arrays & Hashing | `NEETCODE_PLAN_1.md` |
...
- **Фаза 1.7: [NeetCode] Sliding Window (завершается) 🔄 ← Текущий блок — 14/19 основных + 0/2 бонусных решены, осталось 7 задач**
...
- Активно создаются для закрепления паттернов (LC 125, 977, 1732, 724, 202, 15, 643, 167, 283, 680, 1768, 88, 392, 26, 18, 189, 11, 881, 42 и др.)
...
- **The Nature of GO PROGRAMMING** (igoroutine.courses)
  ...
  - **Прогресс:** Лекции (4+) + xv6 SYS_va2pa ✅ + Go assembler (Sum, Fibonacci) ✅ + SIMD slices.Contains ✅ + Продвинутая работа с ошибками ✅
...
- **Telegram-канал** — Видео-разборы алгоритмов
  - **Прогресс:** LC 977, LC 1732, LC 167, LC 125, LC 680, LC 283, LC 1768, LC 392
